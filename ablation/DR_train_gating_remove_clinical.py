...
class ClinicalInfoProcessor(nn.Module):
    ...
    def forward(self, clinical_info):
        # Convert clinical info to structured text
        texts = [
...
class Options:
    def __init__(self):
        ...
        self.epoch = 100
        self.num_class = 2
        self.checkpoint_path = './checkpoints/gating_with2loss_ablation_remove_clinical/'
        self.eval = False
        self.image_size = 384
...
def train_and_evaluate(opt):
    """Training and evaluation function - clinical info removed (set to None)."""
    torch.manual_seed(0)
    np.random.seed(0)
...
def visualize_feature_heatmap(model, data_loader, device, save_path="./visualization_results", num_batches=3):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(data_loader):
            ...
            heatmaps = torch.mean(features, dim=1, keepdim=True)
