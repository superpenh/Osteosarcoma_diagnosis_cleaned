...
def collate_fn(batch):
    ...
    for item in batch:
        ...
        if num_to_pad > 0:
            pad = torch.zeros(
                num_to_pad, curr.size(1), curr.size(2), curr.size(3), dtype=curr.dtype
            )
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch in tqdm(self.dataloader, desc="Testing"):
                ...
                # Filter clinical info to only keep active fields for this ablation experiment
                filtered_clinical_info = []
                for info in batch['clinical_info']:
                    filtered_info = {k: v for k, v in info.items() if k in self.args.active_fields}
                    filtered_clinical_info.append(filtered_info)

                dr_p, patch_p = self.model(
                    batch['dr_images'].to(self.device), batch['patches'].to(self.device), filtered_clinical_info
                )
...
if __name__ == '__main__':
    ...
    ablation_root_dir = '/data/pengxiao/Osteosarcoma_diagnosis/ablation/checkpoints/gating_with2loss_clinicalinfo_bianli0（batch size设为8了不是原来的）'
    ...
    print("=" * 80)
    print("Starting automatic ablation experiment fusion testing")
    print("=" * 80)

    for run_name, fields in ablation_experiments.items():
        print(f"\n\n{'=' * 80}")
        print(f"Testing experiment group: {run_name}")
        print(f"Active clinical features: {fields}")
        print(f"{'=' * 80}")

        search_pattern = os.path.join(ablation_root_dir, run_name, "ddkg_swin_model_gating", "*.pth")
        pth_files = glob.glob(search_pattern)

        if not pth_files:
            print(f"Warning: No weight files found for experiment {run_name}. Skipping. ({search_pattern})")
            continue

        def extract_auc(filepath):
            try:
                return float(filepath.split('_auc')[-1].replace('.pth', ''))
            except:
                return 0.0

        pth_files.sort(key=extract_auc, reverse=True)

        current_checkpoint = pth_files[0]
        print(f"Loading best weight file: {os.path.basename(current_checkpoint)}")

        current_args = copy.deepcopy(base_args)
        current_args.dr_checkpoint = current_checkpoint
        current_args.active_fields = fields

        tester = MultiModalTester(current_args)
        tester.test()
