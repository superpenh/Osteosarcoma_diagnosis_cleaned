...
from classification.train_with_concat import SwinWithFusion
...
class MultiModalFusionModel(nn.Module):
    def __init__(self, dr_model_path, patch_model_path, bert_model_path, num_classes=2):
        ...
        # 2. Patch model uses clinical-aware Swin structure
    ...
    def _init_swin_patch_model(self, checkpoint_path):
        model = SwinWithFusion(num_classes=2, clinical_dim=256, bert_model_path=self.bert_model_path)
        try:
            ...
            print("Successfully loaded Clinical-Aware Patch Swin model checkpoint")
        except Exception as e:
            print(f"Patch model load warning: {e}")
    ...
    def forward(self, dr_images, patches, clinical_info):
        ...
        # --- Patch prediction logic (with dimension alignment) ---
        patch_probs_list = []
        num_patches = patches.size(1)

        for i in range((num_patches + 9) // 10):
            start_idx = i * 10
            end_idx = min((i + 1) * 10, num_patches)
            current_chunk_size = end_idx - start_idx

            p_batch = patches[:, start_idx:end_idx].reshape(-1, 3, 224, 224)
            p_batch = F.interpolate(p_batch, size=(224, 224), mode='bilinear', align_corners=False)

            # Expand clinical_info to match flattened batch dimension
            expanded_clinical_info = []
            for b in range(batch_size):
                expanded_clinical_info.extend([clinical_info[b]] * current_chunk_size)

            patch_output = self.patch_model(p_batch, expanded_clinical_info)
            prob = F.softmax(patch_output['classification'], dim=1).view(batch_size, -1, 2)
...
class Args:
    def __init__(self):
        ...
        self.patch_checkpoint = '/data/pengxiao/Osteosarcoma_diagnosis/classification/checkpoints/swin_with_fusion_ablation/best_model_auc_0.9172_epoch_65.pth'
