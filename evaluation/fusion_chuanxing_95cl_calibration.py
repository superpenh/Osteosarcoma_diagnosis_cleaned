...
def expected_calibration_error(y_true, y_prob, n_bins=10):
    ...
    Partitions predicted probabilities into n_bins intervals and computes
    the weighted absolute difference between mean predicted probability
    and true positive rate in each bin.
    """
    ...
    for i in range(n_bins):
        mask = binids == i
        if np.any(mask):
            prob_true_bin = y_true[mask].mean()
            prob_pred_bin = y_prob[mask].mean()
            ece += (np.abs(prob_true_bin - prob_pred_bin) * np.sum(mask))
...
class MultiModalTester:
    def __init__(self, args):
        ...
        self.model = MultiModalFusionModel(args.dr_checkpoint, args.patch_checkpoint, args.bert_model_path).to(
            self.device)
    ...
    def _calculate_metrics(self, y_true, y_pred, y_prob):
        ...
        return {'Accuracy': acc, 'AUC': auc_score, 'Sensitivity': sens, 'Specificity': spec, 'Precision': prec,
                'NPV': npv, 'F1': f1, 'Brier': brier, 'ECE': ece}
    ...
    def _plot_calibration_curves(self, labels, dr_probs, patch_probs, fusion_probs):
        plt.figure(figsize=(8, 8))

        plt.plot([0, 1], [0, 1], "k:", label="Perfectly calibrated")

        for name, probs in [("Fusion", fusion_probs), ("DR Only", dr_probs), ("WSI Only", patch_probs)]:
            prob_true, prob_pred = calibration_curve(labels, probs, n_bins=5, strategy='quantile')
            plt.plot(prob_pred, prob_true, "s-", label=f"{name}")

        plt.ylabel("Fraction of Positives (Actual Rate)", fontsize=12)
        plt.xlabel("Mean Predicted Probability", fontsize=12)
        plt.title("Calibration Curves (Reliability Diagrams)", fontsize=14)
        plt.legend(loc="lower right", fontsize=10)
        plt.grid(True, alpha=0.3)

        save_path = "calibration_curve_output.png"
    ...
    def _evaluate_fixed_weight(self, labels, dr_probs, patch_probs, names, c_names, dr_weight):
        ...
        header = '-' * 260
        print(header)
        fmt = '{:<18} {:<24} {:<24} {:<24} {:<24} {:<24} {:<24} {:<24} {:<24} {:<24}'

        print(
            fmt.format('Model', 'Acc (95% CI)', 'AUC (95% CI)', 'Sens (95% CI)', 'Spec (95% CI)', 'Prec (95% CI)',
                       'NPV (95% CI)', 'F1 (95% CI)', 'Brier (95% CI)', 'ECE (95% CI)'))
        print(header)
        ...
        for name, ci in [('Fusion (Fixed)', ci_fusion), ('DDKG DR Only', ci_dr), ('WSI Model Only', ci_patch)]:
            print(fmt.format(
                ...
                ci['Precision'], ci['NPV'], ci['F1'], ci['Brier'], ci['ECE']
            ))
        print(header)

        self._plot_calibration_curves(labels, dr_probs, patch_probs, fusion_probs)
...
DATASETS = [
    ...
    {
        'name': 'prospective_val_set',
        'data_root': '/data/pengxiao/Osteosarcoma_diagnosis/data/prospective_val_set_修正DR',
        'os_clinical_file': '/data/pengxiao/Osteosarcoma_diagnosis/data/prospective_val_set_修正DR/2025年 中山附一 前瞻性研究 OS & NOS 2025.XLSX',
        'nos_clinical_file': '/data/pengxiao/Osteosarcoma_diagnosis/data/prospective_val_set_修正DR/2025年 中山附一 前瞻性研究 OS & NOS 2025.XLSX',
    },
    {
        'name': 'full_val_set',
...
CHECKPOINTS = {
    'dr_checkpoint': '/data/pengxiao/Osteosarcoma_diagnosis/radiology/checkpoints/gating_with2loss/全部骨的串行实验/ddkg_swin_model_gating/best_swin_model_epoch33_acc0.789_auc0.864.pth',
    'patch_checkpoint': '/data/pengxiao/Osteosarcoma_diagnosis/checkpoints/classification/2fenlei/swin/best_2fenlei_epoch9_0.835.pth',
    'bert_model_path': "/data/pengxiao/Osteosarcoma_diagnosis/radiology/pretrained_weights/",
    'fixed_dr_weight': 0.25,
}
...
if __name__ == '__main__':
    ...
    for ds in DATASETS:
        print(f"\n{'='*60}")
        print(f"Testing dataset: {ds['name']}")
        print(f"{'='*60}\n")

        class Args:
            def __init__(self):
                self.data_root = ds['data_root']
                self.os_clinical_file = ds['os_clinical_file']
                self.nos_clinical_file = ds['nos_clinical_file']
                self.dr_checkpoint = CHECKPOINTS['dr_checkpoint']
                self.patch_checkpoint = CHECKPOINTS['patch_checkpoint']
                self.bert_model_path = CHECKPOINTS['bert_model_path']
                self.fixed_dr_weight = CHECKPOINTS['fixed_dr_weight']

        args = Args()
        tester = MultiModalTester(args)

        labels, dr_ps, patch_ps = tester.test()
        fusion_probs = np.array(dr_ps) * args.fixed_dr_weight + np.array(patch_ps) * (1 - args.fixed_dr_weight)
    ...
    # Plot all datasets on one calibration curve
    plt.figure(figsize=(8, 8), dpi=300)
    plt.rcParams['font.family'] = 'sans-serif'
    plt.rcParams['font.sans-serif'] = ['Arial']
    plt.plot([0, 1], [0, 1], "k:", label="Perfectly calibrated")

    colors = ['#D65C58', '#E28F43', '#4A86A8', '#656AA6']
    color_order = ['full_val_set', 'CC_val_set', 'external_val_set_1', 'prospective_val_set']
    label_map = {
        'CC_val_set': 'SYSUCC',
        'external_val_set_1': 'YHIO',
        'prospective_val_set': 'FAH-Prospective',
        'full_val_set': 'FAH',
    }
    for name in color_order:
        result = all_results[name]
        color = colors[color_order.index(name)]
        label = label_map.get(name, name)
        prob_true, prob_pred = calibration_curve(
            result['labels'], result['fusion_probs'], n_bins=5, strategy='quantile'
        )
        plt.plot(prob_pred, prob_true, "s-", label=label, color=color, lw=3.0)

    plt.ylabel("Fraction of Positives (Actual Rate)", fontweight='bold', fontsize=14)
    plt.xlabel("Mean Predicted Probability", fontweight='bold', fontsize=14)
    plt.title("Calibration Curves (Fusion Results)", fontweight='bold', fontsize=16, pad=20)
    ...
    # Compute overall result from first 3 datasets (excluding prospective_val_set)
    overall_labels = []
    overall_fusion_probs = []
    for name, result in all_results.items():
        if name != 'prospective_val_set':
            overall_labels.extend(result['labels'])
            overall_fusion_probs.extend(result['fusion_probs'])

    overall_labels = np.array(overall_labels)
    overall_fusion_probs = np.array(overall_fusion_probs)

    print(f"\n{'='*60}")
    print(f"Overall Results (CC + external1 + full_val)")
    print(f"{'='*60}\n")

    dummy_tester = MultiModalTester.__new__(MultiModalTester)
    ci_overall = dummy_tester._calculate_metrics_with_ci(overall_labels, overall_fusion_probs)
    ...
    print(fmt.format(
        'Overall (Fusion)',
        ci_overall['Accuracy'], ci_overall['AUC'], ci_overall['Sensitivity'],
        ci_overall['Specificity'], ci_overall['Precision'], ci_overall['NPV'],
        ci_overall['F1'], ci_overall['Brier'], ci_overall['ECE']
    ))
    print(header)
