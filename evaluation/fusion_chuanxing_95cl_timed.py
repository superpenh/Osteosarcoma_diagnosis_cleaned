"""
fusion_chuanxing_95cl_timed.py
Adds per-patient inference timing on top of fusion_chuanxing_95cl.py.
Prints DR inference time, WSI inference time, and total time after each patient.
"""
import torch
...
class MultiModalFusionModel(nn.Module):
    ...
    def forward(self, dr_images, patches, clinical_info):
        ...
        return dr_prob, patch_prob, dr_time, patch_time
...
def collate_fn(batch):
    ...
    for item in batch:
        ...
        if num_to_pad > 0:
            pad = torch.zeros(
                num_to_pad, curr.size(1), curr.size(2), curr.size(3),
                dtype=curr.dtype
            )
...
class MultiModalTester:
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch in tqdm(self.dataloader, desc="Testing"):
                ...
                patient_label = batch['label'].item()

                if torch.cuda.is_available():
                    torch.cuda.synchronize()
                total_start = time.time()

                dr_p, patch_p, dr_time, patch_time = self.model(
        ...
        print(f"\n{'=' * 100}")
        print(f"Dataset stats: {total_dr_images_count} valid DR images | {len(timing_records)} patients total")
        print('=' * 100)

        if timing_records:
            headers = ["Patient Name", "DR Time (s)", "WSI Time (s)", "Total Time (s)"]
            table = [[
                r['patient_name'],
                f"{r['dr_time']:.3f}",
                f"{r['wsi_time']:.3f}",
            ...
            dr_times = [r['dr_time'] for r in timing_records]
            wsi_times = [r['wsi_time'] for r in timing_records]
            total_times = [r['total_time'] for r in timing_records]
            print(f"\n[Timing Summary]")
            print(f"  DR  - mean: {np.mean(dr_times):.3f}s  |  min: {np.min(dr_times):.3f}s  |  max: {np.max(dr_times):.3f}s")
            print(f"  WSI - mean: {np.mean(wsi_times):.3f}s  |  min: {np.min(wsi_times):.3f}s  |  max: {np.max(wsi_times):.3f}s")
            print(f"  Total - mean: {np.mean(total_times):.3f}s  |  min: {np.min(total_times):.3f}s  |  max: {np.max(total_times):.3f}s")
            print('=' * 100)

            timing_df = pd.DataFrame(timing_records)
            timing_df.to_csv('inference_timing_records.csv', index=False, encoding='utf-8-sig')
            print(f"[Timing] Detailed timing records saved to inference_timing_records.csv")
    ...
    def _evaluate_fixed_weight(self, labels, dr_probs, patch_probs, names, c_names, dr_weight):
        ...
        print(
            fmt.format('Model', 'Accuracy (95% CI)', 'AUC (95% CI)', 'Sens (95% CI)', 'Spec (95% CI)', 'Prec (95% CI)',
                       'NPV (95% CI)', 'F1 (95% CI)'))
        print(header)

        for name, ci in [('Fusion (Fixed)', ci_fusion), ('DDKG DR Only', ci_dr), ('WSI Model Only', ci_patch)]:
            print(fmt.format(
                name, ci['Accuracy'], ci['AUC'], ci['Sensitivity'], ci['Specificity'],
