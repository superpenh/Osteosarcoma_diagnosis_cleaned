...
class SpatialAwareModule(nn.Module):
    ...
    def forward(self, image_features, clinical_embedding=None):
        ...
        # More stable than direct multiplication — avoids feature loss on poor mask init.
...
class MultiModalFusionModel(nn.Module):
    ...
    def _load_dr_checkpoint(self, checkpoint_path):
        try:
            ...
            # Note: strict=False is important due to architecture changes;
            # if loading newly-trained weights, most should match
            ...
            print("Using randomly initialized weights for DR model")
    ...
    def _init_swin_patch_model(self, checkpoint_path):
        ...
        try:
            ...
            print("Using pretrained weights for Swin patch model")
    ...
    def _preprocess_dr_image_for_ddkg(self, dr_image):
        """
        Preprocess DR image for DDKG Swin model.
        Input: [B, C, H, W] (3-channel, 224x224)
        Output: [B, 1, 384, 384] (single-channel, 384x384)
        """
        if dr_image.size(1) == 3:
            dr_image = dr_image.mean(dim=1, keepdim=True)

        dr_image = F.interpolate(
            dr_image,
            size=(384, 384),
            mode='bilinear',
            align_corners=False
    ...
    def forward(self, dr_images, patches, clinical_info):
            ...
            if batch_size == 1:
                ...
                if is_mixed:
                    try:
                        ...
                        # Trigger rule: age > 35 and images disagree → force negative
                        if age > 35:
                            print(f"[Rule Triggered] Age {age} > 35 and mixed DR decisions (Max logic) -> forced negative")
        ...
        # --- Pathology Slice Processing (Swin Model) ---
        patch_probs_list = []
        ...
        # --- Top-K Confidence Selection & Weighting ---
        if patch_probs_list:
...
class MultiModalTester:
    def __init__(self, args):
        ...
        self.dataset = OsteosarcomaDataset(
            args.data_root,
            args.os_clinical_file,
            args.nos_clinical_file,
            image_size=224
        )
        ...
        self.dataset_name = os.path.basename(args.data_root.strip('/').strip('\\'))

    def test(self):
        """Run one inference pass, then either try different weights or use a fixed weight."""
        labels = []
        dr_probs = []
        patch_probs = []
        patient_names = []
        ...
        with torch.no_grad():
            for batch in tqdm(self.dataloader, desc="Testing"):
                ...
                dr_prob, patch_prob = self.model(dr_images, patches, clinical_info)

                labels.append(label)
                dr_probs.append(dr_prob[0][1].item())
    ...
    def _evaluate_fixed_weight(self, labels, dr_probs, patch_probs, patient_names, class_names, fixed_dr_weight):
        ...
        print('{:<20} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f}'.format(
            'Fusion (Fixed)',
            best_metrics['Accuracy'], best_metrics['AUC'], best_metrics['Sensitivity'],
            best_metrics['Specificity'], best_metrics['Precision'], best_metrics['NPV'], best_metrics['F1']
        ))

        print('{:<20} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f}'.format(
            'DDKG Swin DR Only',
            metrics_dr['Accuracy'], metrics_dr['AUC'], metrics_dr['Sensitivity'],
            metrics_dr['Specificity'], metrics_dr['Precision'], metrics_dr['NPV'], metrics_dr['F1']
        ))

        print('{:<20} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f}'.format(
            'WSI Model Only',
            metrics_patch['Accuracy'], metrics_patch['AUC'], metrics_patch['Sensitivity'],
        ...
        if patient_results:
            ...
            self.save_results_to_excel(patient_results, filename_suffix=f"fixed_weight_{fixed_dr_weight:.2f}")
    ...
    def _try_different_weights(self, labels, dr_probs, patch_probs, patient_names, class_names):
        ...
        for result in results:
            ...
            print('{:<10.2f} {:<10.2f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f}'.format(
                result['dr_weight'], result['patch_weight'],
                metrics['Accuracy'], metrics['AUC'], metrics['Sensitivity'],
    ...
    def save_results_to_excel(self, patient_results, filename_suffix=""):
        """Save patient results to an Excel file."""
        ...
        df = df.rename(columns={
            ...
            'fusion_prob': 'Fusion Probability (OS)',
            'dr_prob': 'DR Model Prob (OS)',
            'patch_prob': 'Patch Model Prob (OS)'
        })

        if filename_suffix:
            filename = f"{self.dataset_name}_{filename_suffix}.xlsx"
        else:
            filename = f"{self.dataset_name}_results.xlsx"

        try:
            df.to_excel(filename, index=False)
            print(f"\nResults successfully saved to: {os.path.abspath(filename)}")
        except Exception as e:
